...
- Prepara `.env` con `NODE_ENV=development` (se usa en `build.target` y en el contenedor).
- Arrancar (hot-reload y volumen del código):
  - `docker compose up -d --build`
- Acceder: `http://localhost:${APP_PORT}` (por defecto `http://localhost:3004`).
- Ver logs:
  - App: `docker compose logs -f app`
  - DB: `docker compose logs -f db`
- Reconstrucción recomendada tras cambios en `postcss.config.cjs` o `package*.json`:
...
- El servicio `app` monta `.:/app` y corre `npm run dev` (incluye watcher de CSS: compila `src/styles/**/*.css` → `public/assets/styles`).
- `postcss-nesting` está configurado en `postcss.config.cjs` antes de Tailwind, por lo que el nesting de `.header { &__... }` se aplica.
- Para inspeccionar el CSS generado: `docker compose exec app sh -c 'sed -n "1,120p" public/assets/styles/main.css'`.
- La caché de vistas EJS está desactivada en dev (`server.js:24-28`).
